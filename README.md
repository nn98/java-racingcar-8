...
- Layered Architecture : 관심사를 계층으로 분리해서 관리한다
- MVC Pattern : 관심사를 구성 요소로 분리해서 관리한다 `??`
- 기존 프로젝트는 MVC 패턴으로 구현했으니 그냥 레이어드 아키텍처 아닌가 생각
- 내가 틀렸다는 전제로 깊게 생각해 봤을 때,  
  아키텍처라는 포괄적 범례가 패턴 정도에 귀결될 리는 없다
- Spring 프로젝트를 기준으로 생각해보자
    - Controller : 엔드포인트 관리, 서비스 호출해서 요청 처리, 응답 반환
    - Service : 요청의 데이터 기반으로 작업(비즈니스 로직이겠고) 처리, 컨트롤러에 결과 반환
    - 그 밑의 repo / dto / entity
    - View : 순수 백엔드일 땐 없었지만 webapps의 jsp 파일들. 직접적인 사용자 UI
- 일 때, 소거법으로 모델은 서비스/DTO/Entity 등, 의미상으론 데이터 담는 객체니까 엔티티랑 DTO 아닌가?  
  엔티티에 로직이 포함되기도 하니 서비스도 포함하나? 일단 넘어가고 여기서 레이어(계층)을 생각해 보면
- 사용자에게 보이는 계층과 `아마 Presentation`
- 사용자에게 보여주기 위해 동작하는 계층 `아마 Domain`
- 정도로 구분. 컨트롤러가 보이는 계층인가? 뭐 엔드포인트는 보이니까 presentation에 가깝지 않을까
- 도메인 계층은 관심사로 나눠서 비즈니스 로직을 담당한다 했으니 서비스를 포함하는게 맞고
- 정리하면,
    - 레이어드 아키텍처에선 각각의 관심사에 해당하는 클래스들을 포함하는 계층과
    - MVC 패턴에선 각각의 역할에 해당하는 기능을 포함하는 각 클래스(M, V, C)
- 왜 감은 오는거 같으면서도 표현이 안되냐

---

- 고심은 할만큼 했으니 자료를 탐색해보자
