...
> 컨트롤러 테스트를 구현하려다 문득 컨트롤러가 필요한 미션인가 생각

- 컨트롤러를 자연스럽게 만든 이유는?
    - MVC 패턴은 모델 뷰 컨트롤러 3요소니까
    - 어렴풋이만 알고 있는 모델과 뷰 컨트롤러의 정확한 존재 이유와 목적은?
- [레이어드 아키텍처와 MVC 패턴](https://github.com/nn98/java-racingcar-8/wiki/Layered_Architecture-&-MVC_Pattern)
- ⬆️ 에서의 고뇌와 학습을 끝내고, 이 고민의 근본 원인으로 돌아와보자
- [1주차](https://velog.io/@nn98/%EC%9A%B0%EC%95%84%ED%95%9C%ED%85%8C%ED%81%AC%EC%BD%94%EC%8A%A4-%ED%94%84%EB%A6%AC%EC%BD%94%EC%8A%A4-1%EC%A3%BC%EC%B0%A8-%ED%9A%8C%EA%B3%A0)
  ...
  지금의 프로젝트 구조는 과도한 분리가 [부분](#💡과도한 분리와 설계) [부분](💡커스텀 예외 클래스의 필요성) 보인다
...
> 전에도 그렇고 비즈니스 로직이 처리할 관심사가 하나뿐인데 서비스와 구현체를 분리?

- 이미 분리도 하고 그에 맞춰 테스트도 작성하고 했지만 저 과도한 추상화가 굳이 필요할까?
- 클린 아키텍처라는 허상에 빠져 오버하지 말자
- 구현 통합

### 💡커스텀 예외 클래스의 필요성

> 커스텀 예외는 ProblemDetails 쓰려고 만들었떤건데 이번 미션은 그냥 다 IllegalArgumentException
...
- [X] 사용자 입력값 파싱
    - [X] 쉼표`,`를 기준으로 분할
    - [X] 입력값 검증. 조건에 부합하지 않을 경우 예외 발생, 애플리케이션 종료
        - `조건`
        - [X] 공백이 아님
        - [X] 중복이 아님
        - [X] 길이가 5 이하임
